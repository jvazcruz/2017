			...
			<div class="container">
				<div class="row">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs12">
						<p class="tmat">Retirar Material PDV</p>
					</div>
					<div class="col-lg-8 col-md-8 col-sm-12 col-xs12" id="fg">
						...
						<div class="col-lg-3 col-md-3 col-sm-3 col-xs3">
							  <select name="campania" id="campania" class="form-control">
				                ...
				                <?php
					                if($canal=="Todos")
					                {
					                    $query3="SELECT * FROM `materialpdv_campanias` WHERE `vigencia2` <= '{$dia}' GROUP BY `nombre`";
					                ...
					                {
					                  $q1="SELECT * FROM `materialpdv_imagenes` WHERE `materialpdv_imagenes`.`campania` IN (SELECT `materialpdv_campanias`.`id` FROM `materialpdv_campanias` WHERE `materialpdv_campanias`.`vigencia2` <= '{$dia}') AND `materialpdv_imagenes`.`canales` LIKE '%{$canal}%' GROUP BY `materialpdv_imagenes`.`campania`";
					                  ...
					                  if($totality>0)
					                  {
					                    ...
					                    {
					                      $query3="SELECT * FROM `materialpdv_campanias` WHERE `id`='{$x1['campania']}' AND `vigencia2` <= '{$dia}'";
					                      $result3 = $db->sql_query($query3);
						...
						<div class="col-lg-1 col-md-1 col-sm-1 col-xs1 text-right">
							<button class="descarga" type="button" onclick="window.open('<?php echo '../mautorizadopdv_pdf.php?vigencia=0&campania='.$campania.'&canal='.$canal;?>');" <?php if($totality<=0) echo 'disabled'; ?>><i class="fa fa-arrow-circle-o-down" aria-hidden="true" id="downl"></i></button>
						</div>
					</div>
				</div>
					...
					<div class="col-md-10 col-sm-10 col-xs-10 lienzo">
						...
						<?php
							...
							<div class="col-md-12 col-sm-12 col-xs-12 lienzo_mat">
								<div class="text-center txt_retirar">RETIRAR MATERIAL</div>
								<div class="bg_retirar"></div>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$("#canal").change(function(){
		          ...
		          $(location).attr('href','retirar_material.php?campania='+campania+'&canal='+$(this).val());
		        });
		        $("#campania").change(function(){
		          var canal=$("#canal").val();
		          $(location).attr('href','retirar_material.php?campania='+$(this).val()+'&canal='+canal);
		        });
			});
		</script>
   </body>
